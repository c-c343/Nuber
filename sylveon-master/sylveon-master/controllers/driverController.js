...
router.get("/:driver", (req, res) => {
    User.findOne({name: req.params.driver}, (err, user) => {
        return res.json(user);
    })
})

router.put("/:driver/location", (req, res) => {
    User.findOneAndUpdate({name: req.params.driver}, {position: req.body.location}, (err, driver) => {
        return res.json(driver.position);
    });
});
